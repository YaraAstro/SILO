<script lang="ts">
  ...
  import { Globe, Download, Upload, Search } from "lucide-svelte";
  ...
  let filteredMoreRows = $derived(
    (detailUsage?.sites ?? []).filter((row: DataConsumer) =>
  ...
  function getDetailTotalBytes() {
    return (detailUsage?.sites ?? []).reduce(
  ...
  function getDetailDownloadBytes() {
    return (detailUsage?.sites ?? []).reduce((sum: number, item: DataConsumer) => sum + item.downloadBytes, 0);
  }

  function getDetailUploadBytes() {
    return (detailUsage?.sites ?? []).reduce((sum: number, item: DataConsumer) => sum + item.uploadBytes, 0);
  }
...
<section class="mx-auto max-w-5xl space-y-6" transition:fade={{ duration: 150 }}>
  ...
  <div class="grid gap-4 grid-cols-2 lg:grid-cols-4">
    ...
    <div class="silo-card p-5 flex flex-col justify-between bg-slate-900/40 border border-slate-800/80">
      <span class="text-xs font-bold text-slate-550 uppercase tracking-wider">Active Sites</span>
  ...
  <section class="silo-card p-6 space-y-6">
    ...
    <div class="relative w-full">
      <Search class="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-555" size={17} />
      <input
